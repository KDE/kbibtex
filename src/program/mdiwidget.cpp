...
void MDIWidget::setFile(OpenFileInfo *openFileInfo, KService::Ptr servicePtr)
{
    ...
    if (openFileInfo != NULL) {
        ...
        if (url.isValid())
            emit setCaption(QString(QLatin1String("%1 [%2]")).arg(openFileInfo->shortCaption(), squeeze_text(openFileInfo->fullCaption(), 64)));
        else
...
void MDIWidget::slotCompleted(QObject *obj)
{
    ...
    if (!oldUrl.equals(newUrl)) {
        ...
        emit setCaption(QString(QLatin1String("%1 [%2]")).arg(ofi->shortCaption(), squeeze_text(ofi->fullCaption(), 64)));
    }
}
