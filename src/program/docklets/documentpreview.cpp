...
public:
    ...
    void setupGUI() {
        ...
        if (okularPart == NULL || swpOkular < 0) {
            kWarning() << "No 'KDE 4'-based Okular part for PDF or PostScript document preview available.";
        }
#ifdef HAVE_QTWEBKIT // krazy:exclude=cpp
    ...
    bool showUrl(const struct UrlInfo &urlInfo) {
        static const QStringList okularMimetypes = QStringList() << QLatin1String("application/x-pdf") << QLatin1String("application/pdf") << QLatin1String("application/x-gzpdf") << QLatin1String("application/x-bzpdf") << QLatin1String("application/x-wwf") << QLatin1String("image/vnd.djvu") << QLatin1String("image/vnd.djvu+multipage") << QLatin1String("application/postscript") << QLatin1String("image/x-eps") << QLatin1String("application/x-gzpostscript") << QLatin1String("application/x-bzpostscript") << QLatin1String("image/x-gzeps") << QLatin1String("image/x-bzeps");
        ...
        if (okularPart != NULL && okularMimetypes.contains(urlInfo.mimeType)) {
            ...
            if (urlInfo.mimeType == QLatin1String("application/pdf"))
                additionalInformation = i18nc("Additional information in case there is not KPart available for mime type 'application/pdf'", "<br/><br/>Please install <a href=\"https://userbase.kde.org/Okular\">Okular</a> for KDE&nbsp;4 to make use of its PDF viewing component.<br/>Okular for KDE Frameworks&nbsp;5 will not work.");
