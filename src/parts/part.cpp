...
public:
    ...
    bool saveFile(const KUrl &url) {
        ...
        const QString ending = suffixRegExp.indexIn(url.pathOrUrl().toLower()) > 0 ? suffixRegExp.cap(1) : QLatin1String("bib");
        ...
        if (url.isLocalFile()) {
            ...
            temporaryFile.setSuffix(QLatin1String(".") + ending);
            ...
            if (temporaryFile.open()) {
                ...
                if (result) {
                    ...
                    KIO::NetAccess::del(url, p->widget()); ///< delete target first before copying to its location
        ...
        if (!result) {
            QString msg = i18n("Saving the bibliography to file '%1' failed.", url.pathOrUrl());
            if (errorLog.isEmpty())
