...
public:
    ...
    void makeBackup(const QUrl &url) const {
        ...
        if (!copySucceeded)
            KMessageBox::error(p->widget(), i18n("<qt>Could not create backup copies of document<br/><b>%1</b>.</qt>", url.url(QUrl::PreferLocalFile)), i18n("Backup copies"));
    }
    ...
    bool saveFile(const QUrl &url) {
        ...
        if (suffixRegExp.indexIn(url.url(QUrl::PreferLocalFile)) >= 0)
