...
public:
    ...
    void setupActions()
    {
        ...
        connect(partWidget->fileView(), &FileView::contextMenuTriggered, p, [this](QContextMenuEvent * event) {
            ...
            const auto actions {partWidget->fileView()->actions()};
    ...
    QUrl getSaveFilename(const int options) const {
        ...
        saveDlg->setMimeTypeFilters(supportedMimeTypes.split(u' ', Qt::SkipEmptyParts));
#else // QT_VERSION < 0x050e00
        saveDlg->setMimeTypeFilters(supportedMimeTypes.split(u' ', QString::SkipEmptyParts));
    ...
    bool saveFile(const QUrl &url, const FileScope &fileScope = scopeAllElements) {
        ...
        QScopedPointer<FileExporter> exporter(saveFileExporter(url));
        QStringList errorLog;
        QObject::connect(exporter.data(), &FileExporter::message, p, [&errorLog](const FileExporter::MessageSeverity severity, const QString & messageText) {
        ...
        if (url.isLocalFile()) {
            ...
            if (!fileInfo.exists() || fileInfo.isWritable()) {
                ...
                if (file.open(QIODevice::WriteOnly)) {
                    result = saveFile(file, fileScope, exporter.data());
                    file.close();
                    if (!result)
            ...
            if (temporaryFile.open()) {
                result = saveFile(temporaryFile, fileScope, exporter.data());
