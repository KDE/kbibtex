...
public:
    ...
    bool writeMacro(QTextStream &stream, const QSharedPointer<const Macro> &macro)
    {
    ...
    bool writeComment(QTextStream &stream, const QSharedPointer<const Comment> &comment)
    {
        stream << " <comment>" ;
    ...
    QString valueItemToXML(const QSharedPointer<ValueItem> &valueItem)
    {
...
bool FileExporterXML::save(QIODevice *iodevice, const QSharedPointer<const Element> &element, const File *bibtexfile)
{
    ...
    const bool result = d->write(stream, element);
