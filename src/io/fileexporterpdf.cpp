...
bool FileExporterPDF::save(QIODevice *iodevice, const File *bibtexfile, QStringList *errorLog)
{
    ...
    if (m_fileEmbedding & EmbedBibTeXFile)
        m_embeddedFileList.append(QString(QLatin1String("%1|%2|%3")).arg(QLatin1String("BibTeX source"), m_fileStem + KBibTeX::extensionBibTeX, m_fileBasename + KBibTeX::extensionBibTeX));
...
void FileExporterPDF::fillEmbeddedFileList(const QSharedPointer<const Element> element, const File *bibtexfile)
{
    ...
    if (!entry.isNull()) {
        ...
        foreach(const KUrl &url, urlList) {
            ...
            m_embeddedFileList.append(QString(QLatin1String("%1|%2|%3")).arg(title, filename, basename));
        }
    }
}
