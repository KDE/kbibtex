...
void Groups::finishedFetchingGroups()
{
    ...
    if (reply->error() == QNetworkReply::NoError) {
        ...
        qCWarning(LOG_KBIBTEX_NETWORKING) << "Fetching groups from URL" << reply->url().toDisplayString() << "failed with code" << reply->error() << "and message" << reply->errorString() << "and data" << data.left(64);
        d->busy = false;
        d->initialized = false;
        Q_EMIT finishedLoading();
    }
