...
public:
    ...
    void setupGUI() {
        ...
        QLabel *label = new QLabel(i18n("The following operations can be performed when associating the document about to the entry:"), p);
        ...
        radioMoveFile = new QRadioButton(i18n("Move document next to bibliography file (will delete original document file)"), groupBox);
        ...
        groupBoxRename = new QGroupBox(i18n("Rename?"), p);
        ...
        connect(radioKeepFilename, SIGNAL(clicked()), p, SLOT(updateUIandPreview()));
        ...
        label = new QLabel(i18n("Preview:"), p);
        layout->addWidget(label);

        linePreview = new KLineEdit(i18n("No preview available"), p);
        layout->addWidget(linePreview);
        linePreview->setReadOnly(true);

        layout->addStretch(10);

        p->updateUIandPreview(); /// some radio buttons were set ...
    }
};
...
bool AssociatedFilesUI::associateUrl(const QUrl &url, QSharedPointer<Entry> &entry, File *bibTeXfile, QWidget *parent) {
    ...
    QPointer<AssociatedFilesUI> ui = new AssociatedFilesUI(dlg);
...
AssociatedFilesUI::AssociatedFilesUI(QWidget *parent)
...
void AssociatedFilesUI::updateUIandPreview() {
    d->linePreview->setText(i18n("No preview available"));
    d->groupBoxRename->setEnabled(!d->radioNoCopyMove->isChecked());
    d->groupBoxPathType->setEnabled(!d->radioNoCopyMove->isChecked());
}

void AssociatedFilesUI::setupForRemoteUrl(const QUrl &url, const QString &entryId) {
    d->lineEditSourceUrl->setText(url.toString());
    d->labelGreeting->setText(i18n("The following URL is about to be associated with the entry '%1':", entryId));
}

void AssociatedFilesUI::setupForLocalFile(const QUrl &url, const QString &entryId) {
    d->lineEditSourceUrl->setText(url.path());
    d->labelGreeting->setText(i18n("The following local file is about to be associated with the entry '%1':", entryId));
}
