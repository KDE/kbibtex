...
SortedBibliographyModel::SortedBibliographyModel()
        : QSortFilterProxyModel(), m_sortOrder(SortedBibliographyModel::SortAuthorNewestTitle), model(new BibliographyModel()) {
        ...
        if (role == BibTeXIdRole) {
            ...
            if (isPhdThesis) {
                ...
                if (school.isEmpty()) {
                    ...
                    return i18n("wherepublished-doctoral-dissertation");
                } else {
                    //% "Doctoral dissertation (%1)"
                    return i18n("wherepublished-doctoral-dissertation-arg").arg(school);
                }
            }
            const QString school = valueToText(curEntry->operator[](Entry::ftSchool));
            ...
            case 2:
                //% "%1 and %2"
                return i18n("shortauthors-two-author-args").arg(authors.first()).arg(authors[1]); ///< two authors
            default:
                //% "%1 and %2 more"
                return i18n("shortauthors-one-author-arg-and-n-others-arg").arg(authors.first()).arg(QString::number(authors.size() - 1)); ///< three or more authors
            }
